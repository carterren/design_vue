    ...
    <el-dialog title="添加计划" :visible.sync="centerDialogVisible" width="400px" center>
      <el-form :model="registerForm" ref="registerForm" label-width="80px">
        <el-form-item prop="sProvince" label="始发省市" size="mini">
          <el-cascader size="mini" :options="areaOptions" v-model="selectedOptions.value" @change="handleChange"
        ...
        <el-form-item prop="sProvince" label="目的省市" size="mini">
          <el-cascader size="mini" :options="areaOptions" v-model="selectedOptions1.value" @change="handleChange1"
                       placeholder="请选择省/市/区"/>
        </el-form-item>
  ...
  methods: {
    ...
    exportExcel() {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ['发送省市', '发送详细地址', '收货地址', '收货详细地址', '发送时间','派送车辆','货物名称','货物重量','货物运费','计划创建时间'] // 导出的excel的表头字段
        const filterVal = ['sprovince', 'saddr', 'eprovince', 'eaddr', 'sendDate', 'vehicle', 'cargoName','cargoWeight','price','createTime'] // 对象属性，对应于tHeader
        const list = that.excelData //json数组对象，接口返回的数据
        const data = that.formatJson(filterVal, list)
        export_json_to_excel(tHeader, data, '计划表')// 导出的表格名称
      })
    },
    // 格式转换
